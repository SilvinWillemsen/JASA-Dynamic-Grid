...
The location at where points are added and removed greatly influences the behaviour of the system, especially in the higher frequencies (see Section \ref{sec:results}). The best behaviour is obtained when the location is as close to a boundary as possible. Consider grid functions $u_l^n$ and $w_l^n$ as described before, but now with $M = N - 1$ and $M_w = 1$. This results in grid function $w_l^n$ only having one moving point $w_0^n$, the boundary at $w_1^n$ and virtual grid point $w_2^n$ (see Figure \ref{fig:addingPoint}). Condition \eqref{eq:halfStringBoundaryCondDirichlet} can be extended to be simply supported
\begin{equation}
...
When $c$, and consequently $h$, is decreased and the boundary points surpass the virtual points (i.e. $x_{u_M} \leq x_{w_{-1}}$ and $x_{u_{M+1}} \leq x_{w_0}$) and $N^n > N^{n-1}$, a point approximating $u_{N+1}^n$ is added at the right boundary of $\mathbf{u}^n$ (for both time indices $n$ and $n-1$)
...
\subsection{Static}
In order to determine that the correct frequency is achieved when the system is static, but when interpolation needs to be used (i.e., $\alpha\neq 0$ in Eq. \eqref{eq:connectionInterpol}) the spectrum is plotted 
