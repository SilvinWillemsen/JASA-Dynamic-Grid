...
\end{figure*}
%
\SWcomment[The output sound of the system can be retrieved by following the state $u_l^n$ in Eq. \eqref{eq:updateEq} at any $l$ and listening to that at the given sample rate $f_\text{s}$.] By analysing the scheme in Eq. \eqref{eq:updateEq}, it can be shown that the maximum frequency produced by the system can be calculated using [REF]
...
One way to go from one grid to another is performing a full-grid interpolation \cite[Ch. 5]{bilbao2009Ch5}. If the number of points changes according to Eq. \eqref{eq:orderOfCalcGrid}, i.e., if $N^n \neq N^{n-1}$ the full state of the system ($u_l^n, u_l^{n-1}\ \forall\ l$)  can be interpolated to the new state. See Figure \ref{fig:fullGrid}. 

\begin{figure}
%% \reprintcolumnwidth is the same in preprint and reprint for
%% ease of use for authors:
\includegraphics[width=\reprintcolumnwidth]{fullGrid}
\caption{\label{fig:fullGrid}{Upsampling $u$ (with an arbitrary state) using (linear) full-grid interpolation with $N^{n-1} = 30$ and $N^n = 31$. The x-axis is normalised with respect to $N^n$.}}
\end{figure} 
...
Furthermore, this state-interpolation causes artefacts or `clicks' in the output as sudden variations in the states due to interpolation occur.  
...
The by-product of this is that the CFL condition in \eqref{eq:CFL} can now always be satisfied with equality no matter what the wave speed is.
...
%% ease of use for authors:
\includegraphics[width=\reprintcolumnwidth]{fullGrid}
\caption{\label{fig:fullGrid}{Adding at the boundary}}
\end{figure} 
