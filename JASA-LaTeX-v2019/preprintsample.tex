...
Consider a system, $u$ with $M_u = \text{ceil}(0.5/ck)$ (or simply $M$ below for brevity) and $w$ with $M_w = \text{floor}(0.5/ck)$ points, i.e., half the number of points allowed by the stability condition, plus one for overlap (see Figure \ref{fig:twoFreeStrings}). The total number of points that $u$ and $w$ have are $M + 1$ and $M_{w} + 1$ respectively when including the boundary. Then the following boundary conditions are imposed:
...
\begin{equation}\label{eq:resultOneConnectedPoint}
    ...
    \end{cases}
\end{equation}
which, (again, recalling \eqref{eq:rigid}) are indeed equivalent expressions for the connected point. Here, $w_1$ in the first expression acts as virtual grid point $u_{M+1}^n$ and $u_{M-1}^n$ as virtual grid point $w_{-1}^n$. So essentially, to connect the two strings, the state of one string is used in the update of the other. As with the previous iteration described in \ref{sec:iterations}, the CFL condition in \eqref{eq:CFL} can always be satisfied with equality.

\subsection{Changing the grid}

\begin{figure}[h]
\centerline{\includegraphics[width=\columnwidth]{twoFreeStrings} }
\caption{\label{fig:twoFreeStrings}{Two strings connected at one of their boundaries.}}
\end{figure}
