...
%     \end{bmatrix}.
% \end{equation}
As will be shown in Section \ref{sec:results}, cubic interpolation yields the expected fundamental frequency at all times. It can be seen that when $N$ is an integer, and thus $\alpha = 0$, the system is reduced to \eqref{eq:resultOneConnectedPoint}. % also when interpolation needs to happen.
...
\begin{equation}\label{eq:uMP1Simplified}
...
\end{equation}
where
\begin{equation}\small\label{eq:BMat}
    \mathbf{B} = \begin{bmatrix}[ccccccc|cc]
    & &\ddots &\ddots & \ddots  & & \mathbf{0} & & \\
     & & &1 & 0 & 1 & & \mathbf{0} & \\
     & & &  & 1 & 0 & 1 & & \\
     & &\mathbf{0} &  & \mathbf{A}_{1, 2}\alpha_\text{I} &\mathbf{A}_{1, 2}\beta_\text{I} + 1 &\mathbf{A}_{1, 2}\gamma_\text{I} & \mathbf{A}_{1, 1}(\gamma_\text{I}-\alpha_\text{I})& \\ \cline{2-8}
     & & \mathbf{0} & &\mathbf{A}_{2, 2}\alpha_\text{I} &\mathbf{A}_{2, 2}\beta_\text{I}&\mathbf{A}_{2, 2}\gamma_\text{I} & \mathbf{A}_{2,1}(\gamma_\text{I} - \alpha_\text{I}) & 
    \end{bmatrix}.
\end{equation}
\SWcomment[One can see that if $\alpha \lesssim 1$ in \eqref{eq:interpolationCoeffs}, then $\gamma_\text{I} \approx 1$ and $\alpha_\text{I}\approx \beta_\text{I} \approx \delta_\text{I} \approx 0$. This will reduce $\mathbf{B}$ to a matrix with ones on the diagonals next to the main diagonal and zeros elsewhere.]

Finally, when $M^n > M^{n-1}$ a point is added from $\mathbf{u}^n$ and $\mathbf{u}^{n-1}$ using Eq. \eqref{eq:addingPoint}, and when $M^n < M^{n-1}$ a point is removed from the same vectors using Eq. \eqref{eq:removingPoint}.

\section{Results}\label{sec:results}
This section shows the analysis of the system presented in the previous section and its behaviour.

\subsection{Static}
In order to determine whether the proposed method yields an output with the correct frequency content, a spectrum is taken of the system's output. The system with $N=15.5$ (so $\alpha = 0.5$ in \eqref{eq:interpolationCoeffs}) at a sample rate of $f_\text{s} = 44100$ Hz is compared to the same system with $f_\text{s} = 88200$ Hz resulting in $N=31$ ($\alpha = 0$) for the same $f_0$ according to Eq. \eqref{eq:fundamentalFreq}. See Figure \ref{fig:spectra}.

\begin{figure}[ht]
%% \reprintcolumnwidth is the same in preprint and reprint for
%% ease of use for authors:
\includegraphics[width=\reprintcolumnwidth]{spectraDoubleSampleRate.eps}
\caption{\label{fig:spectra}{A system with $N = 15.5$ at $f_\text{s} = 44100$} compared to a system with $N = 31$ at $f_\text{s} = 88200$ which are supposed to have the same fundamental frequency $f_0$ according to Eq. \eqref{eq:fundamentalFreq}.}
\end{figure} 
...
\subsection{Dynamic}
A modal analysis can be performed on the system. The modal frequency of the $p$'th mode can be retrieved as
\begin{equation}
    f_p = \frac{1}{2\pi k}\cos^{-1}\left(\frac{1}{2}\text{eig}_p(\mathbf{B})\right)
\end{equation}
with $\mathbf{B}$ as defined in Eq. \eqref{eq:BMat}.

As a test case, the wave speed is changed from $c = 2940$ ($N = 15$) to $c = 2205$ ($N = 20$) and the results are shown in Figure \ref{fig:modalAnalysis}. Figure \ref{fig:spectrogram} shows the resulting spectrogram of the system excited at $n=0$ with a narrow raised cosine
