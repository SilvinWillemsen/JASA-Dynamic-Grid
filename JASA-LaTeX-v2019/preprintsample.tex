...
%
Expanding the spatial operators in system \eqref{eq:systemHalfStrings} at $u_M$ and $w_0$, and recalling the conditons in  \eqref{eq:halfStringBoundaryCond} yields
\begin{equation}\label{eq:expandedSystem}
    ...
    \end{cases}
\end{equation}
Because \eqref{eq:rigid} is true, $\delta_{tt}u_M^n = \delta_{tt}w_0^n$, and $F$ can be calculated by setting the equations in \eqref{eq:systemHalfStrings} equal to each other:
\begin{align}
...
\begin{equation}\label{eq:resultOneConnectedPoint}
    ...
    \end{cases}
\end{equation}
which, (again, recalling \eqref{eq:rigid}) are indeed equivalent expressions for the connected point. Here, $w_1$ in the first expression acts as virtual grid point $u_{M+1}^n$ and $u_{M-1}^n$ as virtual grid point $w_{-1}^n$, essentially connecting the two systems using the state of one in the update of the other. As with the previous iteration described in \ref{sec:iterations}, the CFL condition in Eq. \eqref{eq:CFL} can always be satisfied with equality.
...
\caption{\label{fig:twoFreeStringsGridZoomed}{When the grid changes (zoomed). The states at the inner boundaries $u_M^n$ and $w_0^n$ are shown together with virtual grid points $u_{M+1}^n$ and $w_{-1}^n$.}}
\end{figure}

Using $I_1$, informal testing shows that the expected fundamental frequency in Eq. \eqref{eq:fundamentalFreq} is slightly too high when interpolation needed to happen. Instead of a linear interpolation, cubic interpolator $I_3$ can be used according to 
...
\end{gather*}
Treating \eqref{eq:connectionInterpol} as a system of linear equations, the virtual grid point $u_{M+1}^n$ and $w_{-1}^n$ can be solved for using
\begin{equation}
    \begin{bmatrix}
    u_{M+1}^n \\
    w_{-1}^n
...
\begin{equation}
        \begin{cases}\mathbf{u}^n = [\mathbf{u}^n, I_3(x_{u_{M+1}})\mathbf{wu}^n]^T & \text{if $N^n$ is odd}\\
        \mathbf{w}^n = [I_3(x_{w_{-1}})\mathbf{uw}^n, \mathbf{w}^n]^T & \text{if $N^n$ is even}
        \end{cases}
\end{equation}
where 
\begin{gather*}
\mathbf{wu}^n = [w_1^n, w_0^n, u_M^n, u_{M-1}^n] \\
    \mathbf{uw}^n = [u_{M-1}^n, u_M^n, w_0^n, w_1^n]
\end{gather*}
As $\alpha$ in Eq. \eqref{eq:connectionInterpol} is expected to be close to zero \SWcomment[maybe explain?], the second term in either $\mathbf{wu}^n$ or $\mathbf{uw}^n$ is expected to have the biggest contribution to the newly added point. This makes sense by looking at Figure \ref{fig:twoFreeStringsGridZoomed}, as right when the boundary points ($u_M^n$ and $w_0^n$) surpass the virtual points ($u_{M+1}^n$ and $w_{-1}^n$), these are going to be close to overlapping.
