...
\narrowcaption{.25\textwidth}{State $u_l^n$ with $N = 50$ and $f_\text{s} = 44100$ visualised. (A) If $\lambda = 1$, the solution is exact. (B) If $\lambda < 1$ dispersive behaviour shows. (C) If $\lambda > 1$ the stability condition and the system ``explodes".
...
One way to go from one grid to another is performing a full-grid interpolation. Bilbao describes this operation in \cite[Ch. 5]{bilbao2009Ch5}. 
...
The Courant number $\lambda$ will slightly deviate from the CFL condition, in this case, with a minimum of $\lambda \approx 0.9677$. After this, lower values of $c$ yield $N = 31$. According to Eq. \eqref{eq:fmax} $f_\text{max} \approx 18,475 $ Hz. 

but not as much as it would without dynamically adding points to the grid.
