...
\begin{enumerate}
    ...
    \item generate an output with $\text{floor}(N)-1$ modes corresponding to the number of moving points of the system ($p = [1, \hdots, \text{floor}(N)-1]$),\\
...
\begin{subequations}
    \begin{align}
        u_{M+1}^n &= I_1^*(x^n_{u_{M+1}})\mathbf{w}^n = (1-\alpha)w_1^n + \alpha w_0^n
        \\
...
%     \end{bmatrix}.
% \end{equation}
As will be shown in Section \ref{sec:results}, quadratic interpolation yields the expected fundamental frequency at all times. It can be seen that when $N$ is an integer, and thus $\alpha = 0$, the system is reduced to \eqref{eq:resultOneConnectedPoint} (recalling Eq. \eqref{eq:rigid}). % also when interpolation needs to happen.
...
%% \reprintcolumnwidth is the same in preprint and reprint for
%% ease of use for authors:
\includegraphics[width=\reprintcolumnwidth]{addingGridPoint3}
