...
The total state at time $n$ is then denoted as vector $\mathbf{u}^n$ with size $N+1$.
...
This section introduces the proposed method of dynamically changing the grid. To avoid the issues of adding and removing points at the boundary due to boundary conditions, they can be added or removed along the grid instead. For simplicity, the center of the scheme is chosen. In the following, the location of a grid point (in m from the left boundary) $i$ (such as $i = u_0^n$) is denoted by $x_i$. 
...
\end{figure}
%
The systems can then be connected at the inner boundaries ($u_M$ and $w_0$) using a rigid connection, i.e. \SWcomment[(only valid if $x_{u_M^n} = x_{w_0^n}$)],
...
%
Expanding the spatial operators in system \eqref{eq:systemHalfStrings} at the inner boundaries ($u_M^n$ and $w_0^n$), recalling the conditions in  \eqref{eq:halfStringBoundaryCond} and the definition for the virtual grid points needed for the Neumann condition in Eq. \eqref{eq:neumannSolution} yields
\begin{equation}\label{eq:expandedSystem}
    ...
    \end{cases}
\end{equation}
Because of Eq. \eqref{eq:rigid}, $\delta_{tt}u_M^n = \delta_{tt}w_0^n$ is also true, and $F$ can be calculated by setting the \SWcomment[right side of the] equations in \eqref{eq:systemHalfStrings} equal to each other:
\begin{align}
...
When the boundary points surpass the virtual points (i.e. $x_{u_M} \leq x_{w_{-1}}$ and $x_{u_{M+1}} \leq x_{w_0}$) and $N^n > N^{n-1}$, a point is added to the right boundary of $\mathbf{u}$ and the left boundary of $\mathbf{w}$ in an alternating fashion  
...
\end{equation}
This operation is done for both time indices $n$ and $n-1$.

As $\alpha'$ in Eq. \eqref{eq:customIp} is expected to be close to zero \SWcomment[maybe explain?], the third term in either $\mathbf{v}^n$ or $\mathbf{v}_\star^n$ is expected to have the biggest contribution to the newly added point. This makes sense by looking at Figure \ref{fig:twoFreeStringsGridZoomed}, as exactly when the boundary points ($u_M^n$ and $w_0^n$) surpass the virtual points ($u_{M+1}^n$ and $w_{-1}^n$), these are going to be close to overlapping.
